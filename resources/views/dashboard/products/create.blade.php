            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-3 pb-0 pt-1 px-0 me-sm-6 me-5">
                    ...
                    <li class="breadcrumb-item text-sm text-dark active" aria-current="page">
                        {{ __('messages.add_new_products') }}</li>
                </ol>
    ...
    <section class="content">
        ...
        <div class="box box-primary">
            <div class="box-header with border" >
                <h3 class="box-title">{{ __('messages.add_new_products') }}</h3>
            </div>
        ...
        @endforeach
         <div class="form-group">
             ...
             <div class="form-group">
              <img src="{{asset('upload/product_images/default.jpg')}}" style="width:100px;" id="image" class="img-thumbnail image-preview " alt="">
             </div>
             
              <div class="form-group">
               <label for="text">{{__('messages.purchase_price')}}</label>
               <input type="text"class="form-control" name="purchase_price"   placeholder="{{__('messages.purchase_price')}}" value={{old('purchase_price')}}>
             </div>

              <div class="form-group">
               <label for="text">{{__('messages.sale_price')}}</label>
               <input type="text"class="form-control" name="sale_price"   placeholder="{{__('messages.sale_price')}}" value={{old('sale_price')}}>
             </div>

             <div class="form-group">
               <label for="text">{{__('messages.stock')}}</label>
               <input type="text"class="form-control" name="stock"   placeholder="{{__('messages.stock')}}" value={{old('stock')}}>
             </div>             
       <div class="form-group">
         @php
